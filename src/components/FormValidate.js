...
export const FromValidate = yup.object().shape({
    ...
    books: yup.array().of(
        yup.object().shape({
            ...
            chapters: yup.array().length(1, 'Please select at least one chapter of this book!')
        })
    ),
    ...
    }).test('time test', 'Select at least One Slot!', function (value, ctx) {
        Object.keys(value).map(time => {
            if (value[time].length > 0) return true
            else return false
        })
    }),
